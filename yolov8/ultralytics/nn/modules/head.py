...
class RTDETRDecoder(nn.Module):
    def __init__(
        ...
        decoder_layer = DeformableTransformerDecoderLayer(
            hidden_dim, nhead, dim_feedforward, dropout, act, nl, num_decoder_points
        )
        ...
        self.dec_score_head = nn.ModuleList(
            [nn.Linear(hidden_dim, nc) for _ in range(num_decoder_layers)]
        )
    ...
    def _generate_anchors(
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            grid_y, grid_x = torch.meshgrid(
                torch.arange(end=h, dtype=torch.float32),
    ...
    def _get_decoder_input(
        ...
        # Unsigmoided
        reference_points_unact = enc_outputs_coord_unact[batch_ind, topk_ind].view(
        ...
        enc_topk_logits = enc_outputs_class[batch_ind, topk_ind].view(
            bs, self.num_queries, -1
        )
